...
function sendError(response: ServerResponse, statusCode: number, errorCode: string): void {
  sendJson(response, statusCode, { errorCode })
}

async function readJsonBody<T = Record<string, unknown>>(request: IncomingMessage): Promise<T> {
  const chunks: Buffer[] = []
...
function createCharactersApiPlugin(): Plugin {
