...
export const CharacterItemsPrintPage = () => {
    const { t } = useI18n();
    ...
    return (<main className={styles.pageShell}>
      <button className={styles.printButton} type="button" aria-label={t('pages.characterItemsPrint.printButtonLabel')} title={t('pages.characterItemsPrint.printButtonLabel')} onClick={() => window.print()}>
      ...
      <article className={styles.sheet}>
        ...
        <section className={styles.section}>
          {hasItems ? (<div className={styles.sectionStack}>
              {weapons.length > 0 ? (<section className={styles.sectionGroup}>
                  <h2 className={styles.sectionTitle}>{t('pages.characterItemsPrint.sections.weapons')}</h2>
                  <div className={styles.itemGrid}>
                    {weapons.map((item) => (<article key={item.key} className={styles.itemCard}>
              ...
              {armors.length > 0 ? (<section className={styles.sectionGroup}>
                  <h2 className={styles.sectionTitle}>{t('pages.characterItemsPrint.sections.armors')}</h2>
                  <div className={styles.itemGrid}>
                    {armors.map((item) => (<article key={item.key} className={styles.itemCard}>
                        <h3 className={styles.itemName}>
                          <AppIcon className={styles.itemIcon} name={getItemIconName(item.category)}/>
                          <span>{item.name}</span>
                        </h3>
              ...
              {others.length > 0 ? (<section className={styles.sectionGroup}>
                  <h2 className={styles.sectionTitle}>{t('pages.characterItemsPrint.sections.others')}</h2>
                  <div className={styles.itemGrid}>
                    {others.map((item) => (<article key={item.key} className={styles.itemCard}>
                        <h3 className={styles.itemName}>
                          <AppIcon className={styles.itemIcon} name={getItemIconName(item.category)}/>
                          <span>{item.name}</span>
                        </h3>
            ...
            </div>) : (<p className={styles.emptyState}>{t('pages.characterItemsPrint.emptyState')}</p>)}

          <div className={styles.sectionGroup}>
            <div className={styles.extraGrid}>
              {extraSlots.map((label, index) => (<article key={`${label || 'empty'}-${index}`} className={styles.extraSlot}>
                  {label ? (<h3 className={styles.itemName}>
