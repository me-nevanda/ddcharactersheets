...
import { buildDefenseValues, clampDefenseValue } from '@pages/CharacterEditPage/sections/DefensesSection/defensesSectionHooks';
import { buildSkillBonuses } from '@pages/CharacterEditPage/sections/SkillSection/skillSectionHooks';
...
const buildEquippedItemAttributeBonuses = (items: CharacterItems) => {
    return {
...
const normalizeSkillTraining = (training: Partial<CharacterTraining> | undefined): CharacterTraining => {
    return {
        ...
        dungeons: training?.dungeons ?? false,
...
export const useCharacterPrintPage = (): CharacterPrintPageState => {
    const { t } = useI18n();
    const { characterId = '' } = useParams();
    const { character, loading, error } = useCharacterById(characterId, t);
    useCharacterDocumentTitle(t('pages.characterPrint.title'), character?.name);
    const computedState = useMemo<CharacterPrintPageState>(() => {
        ...
        const skillBonuses = buildSkillBonuses(character.level, attributeModifierMap, skillTraining, character.race, character.items);
        const skillBonusesWithFeats: CharacterSkillBonuses = {
            ...
            dungeons: skillBonuses.dungeons + sumFeatBonus(feats, 'dungeonsBonusNumber'),
        ...
        const defenseValues = buildDefenseValues(attributeModifierMap, levelBonus, character.race, character.class, equippedDefenseBonuses);
