  ...
  return (
    <div className={styles.section}>
      <div className={styles.sectionHeader}>
        <h2 className={styles.sectionTitle}>{t('pages.monsterEdit.loot.title')}</h2>
      </div>

      <div className={styles.itemsGroupTabsRow}>
        <div className={styles.itemsGroupTabs} role="tablist" aria-label={t('pages.monsterEdit.loot.title')}>
          {itemGroups.map((group) => (
            <button key={group} id={`monster-loot-tab-${group}`} className={`${styles.itemsGroupTabButton} ${activeGroup === group ? styles.itemsGroupTabButtonActive : ''}`} type="button" role="tab" aria-selected={activeGroup === group} aria-controls={`monster-loot-panel-${group}`} onClick={() => setActiveGroup(group)}>
              {t(`pages.monsterEdit.loot.groups.${group}`)}
            </button>
          ))}
        </div>
        ...
        <button className={styles.primaryButton} type="button" onClick={() => onItemCreateEmpty(activeGroup)}>
          <span className={styles.buttonContent}>
            <AppIcon name="plus" />
            <span>{t('pages.monsterEdit.loot.addButton')}</span>
          </span>
        </button>
      </div>

      <section className={styles.section} id={`monster-loot-panel-${activeGroup}`} role="tabpanel" aria-labelledby={`monster-loot-tab-${activeGroup}`}>
        {activeItems.length === 0 ? <p className={styles.loadingText}>{t('pages.monsterEdit.loot.emptyState')}</p> : null}
        ...
        <div className={styles.deleteBackdrop} role="presentation">
          <div className={styles.deleteDialog} role="dialog" aria-modal="true" aria-labelledby="delete-loot-title">
            <h2 className={styles.deleteDialogTitle} id="delete-loot-title">
              {t('pages.monsterEdit.loot.removeDialog.title')}
            </h2>
            <p className={styles.deleteDialogText}>
              {t('pages.monsterEdit.loot.removeDialog.body', {
