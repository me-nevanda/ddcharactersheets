...
const pickBoostedDefenseFields = (): Set<keyof MonsterDefenses> => {
...
const buildRoleHp = (level: number, rules: MonsterRoleAttributeRules): number => {
...
const boostRandomDefenses = (defenses: MonsterDefenses): MonsterDefenses => {
  const boostedFields = pickBoostedDefenseFields()

  return defenseFields.reduce<MonsterDefenses>((nextDefenses, fieldName) => ({
    ...nextDefenses,
    [fieldName]: nextDefenses[fieldName] + (boostedFields.has(fieldName) ? 2 : 0),
  }), defenses)
}

export const useMonsterAttributeGeneration = () => {
  const generateMonsterAttributes = useCallback((level: number, role: MonsterRole, type: MonsterType): GeneratedMonsterAttributes => {
