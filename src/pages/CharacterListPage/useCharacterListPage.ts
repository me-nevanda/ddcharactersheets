...
import type { CharacterListCardViewModel, CharacterListPageState } from './types'
...
export function useCharacterListPage(): CharacterListPageState {
  ...
  const cards: CharacterListCardViewModel[] = characters.map((character) => ({
    id: character.id,
  ...
  return {
    cards,
    creating,
    deleteDialogCharacterName: getCharacterLabel(characterToDelete, t),
    deletingId,
    error,
