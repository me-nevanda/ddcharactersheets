  ...
  return (
    <section className={styles.section}>
      <div className={styles.sectionHeader}>
        <h2 className={styles.sectionTitle}>{t('pages.npcEdit.attacks.title')}</h2>
      </div>

      <div className={styles.itemsGroupTabsRow}>
        <div className={styles.itemsGroupTabs} role="tablist" aria-label={t('pages.npcEdit.attacks.typeLabel')}>
          {attackTypeTabs.map((tab) => (
            <button key={tab.value} id={`npc-attacks-tab-${tab.value}`} className={`${styles.itemsGroupTabButton} ${activeType === tab.value ? styles.itemsGroupTabButtonActive : ''}`} type="button" role="tab" aria-selected={activeType === tab.value} aria-controls={`npc-attacks-panel-${tab.value}`} onClick={() => setActiveType(tab.value)}>
              {tab.label}
            </button>
          ))}
        </div>
        ...
        <button className={styles.primaryButton} type="button" onClick={handleAddAttack}>
          <span className={styles.buttonContent}>
            <AppIcon name="plus" />
            <span>{t('pages.npcEdit.attacks.addButton')}</span>
          </span>
        </button>
      </div>

      <div id={`npc-attacks-panel-${activeType}`} role="tabpanel" aria-labelledby={`npc-attacks-tab-${activeType}`}>
        {visibleAttacks.length === 0 ? <p className={styles.loadingText}>{t('pages.npcEdit.attacks.emptyState')}</p> : null}
