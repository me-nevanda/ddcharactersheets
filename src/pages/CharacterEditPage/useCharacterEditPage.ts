...
import { emptyAbilities, emptyArmor, emptyFeat, emptyFeats, emptyOtherItem, emptyItems, emptyWeapon, emptyForm, emptyTraining, defaultAbilityAction, defaultAbilityKind, defaultAbilityType, defaultAbilityWeaponDamageDiceCount, defaultAbilityWeaponDamageDiceType, defaultAbilityWeaponDamageType, defaultAbilityWeaponRecurringDamageCount, defaultAbilityWeaponRecurringDamageType, defaultAbilityWeaponAttackAttribute, defaultAbilityWeaponAttackBonusNumber, defaultAbilityWeaponAttackDefense, defaultAbilityWeaponHit, defaultAbilityWeaponMiss, defaultAbilityWeaponProvocation, defaultAbilityWeaponRange, defaultAbilityWeaponArea, zeroAttributeBonuses, zeroDefenses, zeroDefenseBonuses, } from './characterEditPageDefaults';
...
import { CharacterAlignment, CharacterClass, type CharacterAbility, type CharacterAbilityAreaType, type CharacterAbilityType, type CharacterAttributeBonuses, type CharacterBonuses, type CharacterArmorBonusFieldName, type CharacterArmor, type CharacterDefenses, type CharacterFeat, CharacterGender, type CharacterWeapon, type CharacterOtherItem, type CharacterItems, type CharacterItemBonusFieldName, type CharacterWeaponFieldName, type CharacterWeaponDamageDiceType, type CharacterWeaponDamageType, type CharacterSkillBonuses, } from '../../types/character';
...
const normalizeAbilityWeaponDamageDiceType = (value: unknown, fallback: CharacterWeaponDamageDiceType | ''): CharacterWeaponDamageDiceType | '' => {
    if (value === '' ||
        value === 'd4' ||
        value === 'd6' ||
        value === 'd8' ||
...
const normalizeAbilityWeaponDamageDiceCount = (value: unknown, fallback: number): number => {
...
const normalizeAbilityWeaponRecurringDamageCount = (value: unknown, fallback: number): number => {
    if (typeof value === 'number' && Number.isFinite(value)) {
...
const normalizeAbilityWeaponDamageType = (value: unknown, fallback: CharacterWeaponDamageType): CharacterWeaponDamageType => {
    if (value === 'normal' ||
        value === 'acid' ||
        value === 'cold' ||
        value === 'fire' ||
...
const normalizeAbilityWeaponAttackAttribute = (value: unknown): CharacterAbility['weaponAttackAttribute'] => {
    if (value === 'strength' ||
        value === 'condition' ||
        value === 'dexterity' ||
        value === 'intelligence' ||
...
const normalizeAbilityWeaponAttackDefense = (value: unknown): CharacterAbility['weaponAttackDefense'] => {
...
const normalizeAlignmentValue = (value: unknown): CharacterAlignment => {
    if (value === CharacterAlignment.LawfulGood ||
...
const normalizeAbilityWeaponAttackBonusNumber = (value: unknown): number => {
    if (typeof value === 'number' && Number.isFinite(value)) {
...
const normalizeAbilityType = (value: unknown): CharacterAbility['type'] => {
    if (value === 'standard' || value === 'unlimited' || value === 'encounter' || value === 'daily') {
        return value;
    }
    return defaultAbilityType;
};
const normalizeAbilityWeaponRange = (value: unknown): number => {
    if (typeof value === 'number' && Number.isFinite(value)) {
...
const normalizeAbilityWeaponArea = (value: unknown): CharacterAbilityAreaType => {
    if (value === 'point' ||
        value === 'burst1' ||
        value === 'burst2' ||
        value === 'burst3' ||
...
const clampDefenseValue = (value: number): number => {
    return Math.min(30, Math.max(0, Math.trunc(value)));
};
const hasFeatContent = (feat: CharacterFeat): boolean => {
    return (feat.name.length > 0 ||
...
const normalizeFeats = (data: unknown): CharacterFeat[] => {
...
const normalizeWeaponRange = (value: unknown): number => {
    if (typeof value === 'number' && Number.isFinite(value)) {
...
const normalizeWeaponBonusNumber = (value: unknown): number => {
    if (typeof value === 'number' && Number.isFinite(value)) {
        return Math.min(10, Math.max(-5, Math.trunc(value)));
    }
    if (typeof value === 'string') {
...
const normalizeWeaponProficiencyBonusNumber = (value: unknown): number => {
    if (typeof value === 'number' && Number.isFinite(value)) {
...
const buildEquippedItemBonusSources = (items: CharacterItems, fieldName: CharacterItemBonusFieldName): EquippedItemBonusSource[] => {
...
const buildItemAttributeBonuses = (items: CharacterItems): CharacterAttributeBonuses => {
    return {
...
const buildItemDefenseBonuses = (items: CharacterItems): CharacterDefenses => {
    return {
...
const buildItemSpeedBonus = (items: CharacterItems): number => {
...
const buildSourceTooltipLine = (label: string, sources: EquippedItemBonusSource[], formatter: (value: number) => string = formatModifier): string => {
    if (sources.length === 0) {
        return '';
    }
    ...
    const normalizeItems = (items: unknown): CharacterItems => {
        ...
        const normalizeWeaponGroup = (group: unknown): CharacterWeapon[] => {
            ...
            return group
                ...
                name: typeof entry.name === 'string' ? entry.name : '',
                description: typeof entry.description === 'string' ? entry.description : '',
                damageDiceCount: typeof entry.damageDiceCount === 'number' && Number.isFinite(entry.damageDiceCount)
...
const buildAttributeTooltip = (raceLabel: string, raceName: string, raceBonus: number, itemLabel: string, itemSources: EquippedItemBonusSource[]): string => {
...
const buildSpeedTooltip = (baseLabel: string, baseSpeed: number, itemLabel: string, itemSources: EquippedItemBonusSource[], featLabel: string, featSources: EquippedItemBonusSource[]): string => {
...
const buildHpTooltip = (hpLabel: string, hpValue: number, featLabel: string, featSources: EquippedItemBonusSource[]): string => {
...
const buildDefenseTooltip = (defenseLabel: string, levelLabel: string, raceLabel: string, classLabel: string, attributesLabel: string, itemsLabel: string, featLabel: string, raceName: string, characterClassLabel: string, levelBonus: number, raceBonus: number, classBonus: number, attributeBonus: number, itemSources: EquippedItemBonusSource[], featSources: EquippedItemBonusSource[], attributeLabels: string[]): string => {
    const lines = [
        defenseLabel,
