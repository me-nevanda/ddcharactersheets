...
const featCoreFields = [
    ...
    fieldName: CharacterFeatBonusFieldName;
    labelKey: string;
}>;
...
export const FeatsTab = () => {
    const { t, form, handleFeatChange, handleFeatBonusFieldChange, pendingRemoval, handleAddFeat, handleRemoveFeat, handleConfirmRemoveFeat, handleCancelRemoveFeat, } = useFeatsTab();
    return (<div className={styles.section}>
      ...
      {form.feats.length > 0 ? (<div className={localStyles.featsGrid}>
          {form.feats.map((feat, index) => (<article key={feat.id} className={styles.abilityCard}>
              <div className={`${styles.sectionHeader} ${localStyles.featCardHeader}`}>
                <input className={styles.abilityCardTitleInput} id={`feat-name-${index}`} value={feat.name} placeholder={t('pages.characterEdit.feats.namePlaceholder')} onChange={(event) => handleFeatChange(index, 'name', event.target.value)}/>
                <div className={styles.itemCardActions}>
                  <button className={`${styles.weaponEquipButton} ${feat.visible ? styles.weaponEquipButtonActive : styles.weaponEquipButtonInactive}`} type="button" aria-pressed={feat.visible} aria-label="Widoczność na wydruku" title="Widoczność na wydruku" onClick={() => handleFeatChange(index, 'visible', !feat.visible)}>
                    <AppIcon name={feat.visible ? 'check' : 'circle'}/>
                  </button>
                  <button className={styles.abilityRemoveButton} type="button" aria-label={t('pages.characterEdit.feats.removeButton')} title={t('pages.characterEdit.feats.removeButton')} onClick={() => handleRemoveFeat(index)}>
                    <AppIcon name="delete"/>
                  </button>
                </div>
              </div>
              ...
              <div className={localStyles.featCardBody}>
                <label className={localStyles.featDescriptionField} htmlFor={`feat-description-${index}`}>
                  ...
                  <textarea className={`${styles.input} ${styles.abilityTextarea}`} id={`feat-description-${index}`} value={feat.description} placeholder={t('pages.characterEdit.feats.descriptionPlaceholder')} onChange={(event) => handleFeatChange(index, 'description', event.target.value)}/>
                </label>

                <ItemBonusEditor bonusFields={featBonusFields} getBonusValue={(fieldName) => feat[fieldName]} idPrefix={`feat-${index}`} onBonusFieldChange={(previousFieldName, nextFieldName) => handleFeatBonusFieldChange(index, previousFieldName, nextFieldName)} onBonusValueChange={(fieldName, value) => handleFeatChange(index, fieldName, value)}/>
              </div>
            </article>))}
        </div>) : null}
