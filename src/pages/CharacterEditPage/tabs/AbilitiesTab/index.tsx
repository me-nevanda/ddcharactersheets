    ...
    return (<div className={styles.section}>
      ...
      <div className={styles.itemsGroupTabsRow}>
        <div className={styles.itemsGroupTabs} role="tablist" aria-label={t('pages.characterEdit.abilities.typeLabel')}>
          {abilityTypeTabs.map((tab) => (<button key={tab.value} id={`abilities-tab-${tab.value}`} className={`${styles.itemsGroupTabButton} ${activeType === tab.value ? styles.itemsGroupTabButtonActive : ''}`} type="button" role="tab" aria-selected={activeType === tab.value} aria-controls={`abilities-panel-${tab.value}`} onClick={() => setActiveType(tab.value)}>
              {tab.label}
            </button>))}
        </div>
      ...
      <div id={`abilities-panel-${activeType}`} role="tabpanel" aria-labelledby={`abilities-tab-${activeType}`}>
        {visibleAbilities.length === 0 ? <p className={styles.loadingText}>{t('pages.characterEdit.abilities.emptyState')}</p> : null}

        {visibleAbilities.length > 0 ? (<div className={styles.abilityGrid}>
            {visibleAbilities.map(({ ability, index }) => (<AbilityCard key={ability.id} ability={ability} index={index} attributeOptions={attributeOptions} attackBonusOptions={attackBonusOptions} defenseOptions={defenseOptions} weaponAreaOptions={weaponAreaOptions} weaponDamageTypeOptions={weaponDamageTypeOptions} weaponOptions={weaponOptions} getAbilityHeaderClass={getAbilityHeaderClass} onAbilityChange={handleAbilityChange} onRemoveAbility={handleRemoveAbility} t={t}/>))}
          </div>) : null}
      </div>

      <AbilityRemoveDialog pendingRemoval={pendingRemoval} onCancel={handleCancelRemoveAbility} onConfirm={handleConfirmRemoveAbility} t={t}/>
    </div>);
};
