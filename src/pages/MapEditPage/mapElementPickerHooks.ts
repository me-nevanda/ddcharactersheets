...
export const getMapElementVariantLabel = (
  ...
  const variantLabels = category === 'trees'
    ? treeVariantLabels
    : category === 'bushes'
      ? bushVariantLabels
      : category === 'stairs'
        ? stairVariantLabels
        : category === 'furnitures'
          ? furnitureVariantLabels
          : category === 'stones'
            ? stoneVariantLabels
            : category === 'monsters'
              ? monsterVariantLabels
              : category === 'misc'
                ? miscVariantLabels
                : {}

  return variantLabels[variant] ?? t('pages.mapEdit.elementAssetLabel', { category: categoryLabel, number: variant })
}
  ...
  return useMemo(() => elementCategories.map((category) => {
    ...
    return {
      ...
      options: elementAssetOptions.filter((option) => option.category === category).map((option) => ({
        ...option,
        label: getMapElementVariantLabel(option.category, option.variant, t),
      })),
    }
