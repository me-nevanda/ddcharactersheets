import type { TranslationDictionary } from '../types'

export const en: TranslationDictionary = {
  localeName: 'English',
  common: {
    language: 'Language',
    languages: {
      pl: 'PL',
      en: 'EN',
    },
    actions: {
      addCard: 'Add card',
      cancel: 'Cancel',
      delete: 'Delete',
      edit: 'Edit',
      print: 'Print',
      save: 'Save',
      backToList: 'Back to list',
    },
    editor: {
      bold: 'Bold',
      italic: 'Italic',
      underline: 'Underline',
      bulletList: 'Bullet list',
      numberedList: 'Numbered list',
      clearFormatting: 'Clear formatting',
    },
    confirmations: {
      unsavedChangesTitle: 'Unsaved changes',
      unsavedChangesBody: 'You have unsaved changes. Are you sure you want to return to the list?',
    },
    clipboard: {
      copyDamageValue: 'Copy damage marker',
      damageCopied: 'Damage marker copied.',
      copyFailed: 'Could not copy to clipboard.',
    },
    states: {
      creating: 'Creating...',
      deleting: 'Deleting...',
      loadingCards: 'Loading cards...',
      loadingCharacter: 'Loading character data...',
      loadingMonsters: 'Loading monsters...',
      saving: 'Saving...',
    },
  },
  errors: {
    api: {
      generic: 'The request could not be completed.',
      invalidCharacterId: 'The character ID is invalid.',
      invalidCharacterGroupId: 'The hero group ID is invalid.',
      invalidMonsterId: 'The monster ID is invalid.',
      invalidMonsterGroupId: 'The monster group ID is invalid.',
      invalidNpcId: 'The NPC ID is invalid.',
      invalidNpcGroupId: 'The NPC group ID is invalid.',
      invalidAdventureId: 'The adventure ID is invalid.',
      invalidAreaId: 'The area ID is invalid.',
      invalidEventId: 'The event ID is invalid.',
      invalidMapId: 'The map ID is invalid.',
      invalidContextId: 'The context ID is invalid.',
      invalidJsonBody: 'The request payload is invalid.',
      invalidCharacterImage: 'The hero image must be a PNG or JPG file.',
      invalidCharacterGroupName: 'The hero group name is required.',
      invalidMonsterImage: 'The monster image must be a PNG or JPG file.',
      invalidNpcImage: 'The NPC image must be a PNG or JPG file.',
      invalidAreaImage: 'The area image must be a PNG or JPG file.',
      invalidEventImage: 'The event image must be a PNG or JPG file.',
      invalidContextImage: 'The context image must be a PNG or JPG file.',
      invalidMonsterGroupName: 'The monster group name is required.',
      invalidNpcGroupName: 'The NPC group name is required.',
      geminiPromptRequired: 'The Gemini prompt is required.',
      geminiConfigMissing: 'The config.json file with Gemini configuration is missing.',
      geminiConfigInvalid: 'The config.json file format is invalid.',
      geminiApiKeyMissing: 'The Gemini API key is missing in config.json.',
      geminiUnauthorized: 'The Gemini API key is invalid or does not have the required permissions.',
      geminiRateLimited: 'The Gemini request limit was reached. Try again shortly.',
      geminiRequestFailed: 'The Gemini request could not be completed.',
      characterNotFound: 'Character not found.',
      monsterNotFound: 'Monster not found.',
      npcNotFound: 'NPC not found.',
      adventureNotFound: 'Adventure not found.',
      areaNotFound: 'Area not found.',
      eventNotFound: 'Event not found.',
      mapNotFound: 'Map not found.',
      contextNotFound: 'Context not found.',
      notFound: 'Resource not found.',
      unexpectedServerError: 'An unexpected server error occurred.',
    },
  },
  pages: {
    main: {
      subtitle: 'Game master helper',
      tabsLabel: 'Sheet list',
      tabs: {
        heroes: 'Heroes',
        monsters: 'Monsters',
        npcs: 'NPC',
        adventures: 'Adventures',
        areas: 'Areas',
        events: 'Events',
        maps: 'Maps',
        contexts: 'Contexts',
      },
    },
    areaList: {
      actions: {
        addArea: 'New area',
      },
      emptyState: 'There are no areas yet. Add one to start editing.',
      emptySearchState: 'No areas match your search.',
      loading: 'Loading areas...',
      searchLabel: 'Search areas',
      searchPlaceholder: 'Search by area name',
      unnamedArea: 'Unnamed area',
      updatedAt: 'Last updated:',
      deleteDialog: {
        title: 'Delete this area?',
        body: 'The area "{name}" will be deleted permanently.',
      },
    },
    eventList: {
      actions: {
        addEvent: 'Add event',
      },
      emptyState: 'There are no events yet. Add one to start editing.',
      emptySearchState: 'No events match your search.',
      loading: 'Loading events...',
      searchLabel: 'Search events',
      searchPlaceholder: 'Search by event name',
      unnamedEvent: 'Unnamed event',
      updatedAt: 'Last updated:',
      deleteDialog: {
        title: 'Delete this event?',
        body: 'The event "{name}" will be deleted permanently.',
      },
    },
    eventEdit: {
      contextCopyError: 'Could not copy event context.',
      contextCopySuccess: 'Event context copied.',
      copyingContextButton: 'Copying...',
      copyContextButton: 'Context',
      eyebrow: 'EVENT',
      loading: 'Loading event...',
      fields: {
        name: 'Title',
        description: 'Description',
        image: 'Event image',
      },
      placeholders: {
        titleName: 'Unnamed event',
        description: 'Event description',
        image: 'Add image',
      },
      imageActions: {
        uploadNew: 'Change',
        remove: 'Remove',
        removeDialog: {
          title: 'Remove event image?',
          body: 'The "{name}" will be removed from this event.',
        },
      },
    },
    mapList: {
      actions: {
        addMap: 'Add map',
      },
      emptyState: 'There are no maps yet. Add one to start editing.',
      emptySearchState: 'No maps match your search.',
      loading: 'Loading maps...',
      searchLabel: 'Search maps',
      searchPlaceholder: 'Search by map name',
      unnamedMap: 'Unnamed map',
      updatedAt: 'Last updated:',
      deleteDialog: {
        title: 'Delete this map?',
        body: 'The map "{name}" will be deleted permanently.',
      },
    },
    mapEdit: {
      contextCopyError: 'Could not copy map context.',
      contextCopySuccess: 'Map context copied.',
      copyingContextButton: 'Copying...',
      copyContextButton: 'Context',
      drawModeLabel: 'Drawing mode',
      eyebrow: 'MAP',
      emptyLayer: 'This layer is empty for now.',
      enterFullscreen: 'Fullscreen',
      editorLabel: 'Map editor',
      exitFullscreen: 'Exit fullscreen',
      layersLabel: 'Map layers',
      loading: 'Loading map...',
      paletteLabel: 'Map colors',
      groundPaletteLabel: 'Grounds',
      groundTextureLabel: 'Ground {number}',
      groundTextureVariants: {
        grass: 'Grass',
        water: 'Water',
        sand: 'Sand',
        woodenFloor: 'Wooden floor',
        gravel: 'Gravel',
        cobblestone: 'Cobblestone',
        ice: 'Ice',
        lava: 'Lava',
        stoneFloor: 'Stone floor',
        parquet: 'Parquet',
        stonyGround: 'Stony ground',
        woodenTerrace: 'Wooden terrace',
        swamp: 'Swamp',
        crackFissure: 'Crack/Fissure',
        unevenTerrain: 'Uneven terrain',
      },
      elementPaletteLabel: 'Elements',
      elementCategoryLabel: '{category}',
      elementAssetLabel: '{category} {number}',
      elementVariantHint: 'Double-click an element to choose a different type.',
      toggleElementLabel: 'Toggle map element',
      toggleGroundCellLabel: 'Toggle ground',
      toggleLabelLabel: 'Toggle map label',
      toggleLineLabel: 'Toggle map line',
      labelNameInputLabel: 'Label name',
      confirmLabelNameLabel: 'Confirm label name',
      layers: {
        lines: 'Lines',
        ground: 'Ground',
        elements: 'Elements',
        labels: 'Labels',
      },
      drawModes: {
        single: 'Single',
        range: 'Segment',
        rectangle: 'Rectangle',
      },
      colors: {
        black: 'Black',
        red: 'Red',
        green: 'Green',
        blue: 'Blue',
        white: 'White',
        gray: 'Gray',
        yellow: 'Yellow',
        orange: 'Orange',
        purple: 'Purple',
        brown: 'Brown',
        tortoise: 'Tortoise',
        pink: 'Pink',
      },
      elementCategories: {
        trees: 'Tree',
        bushes: 'Bush',
        stairs: 'Stairs',
        furnitures: 'Furniture',
        stones: 'Stones',
        monsters: 'Monsters',
        misc: 'Misc',
      },
      elementTreeVariants: {
        deciduous: 'Deciduous tree',
        coniferous: 'Coniferous tree',
        fantasy: 'Fantasy tree',
        modeled: 'Modeled tree',
        fallen: 'Fallen tree',
        stump: 'Stump',
      },
      elementBushVariants: {
        regular: 'Regular bush',
        yellowFlowers: 'Bush with yellow flowers',
        whiteFlowers: 'Bush with white flowers',
        threeSmall: 'Three small bushes',
        hedge: 'Hedge',
        plants: 'Small plants',
        twoSmall: 'Two small bushes',
        fruit: 'Fruit bush',
      },
      elementStairVariants: {
        upDownStairs: 'Up/down stairs',
        leftRightStairs: 'Left/right stairs',
        ladderUp: 'Ladder up',
        ladderDown: 'Ladder down',
        entranceUp: 'Entrance up',
        descentDown: 'Descent down',
      },
      elementFurnitureVariants: {
        shelf: 'Shelf',
        cabinet: 'Cabinet',
        tableDesk: 'Table/Desk',
        bed: 'Bed',
        couch: 'Couch',
        armchair: 'Armchair',
        chair: 'Chair',
        fireplace: 'Fireplace',
        shopCounter: 'Shop counter',
        smallTable: 'Small table',
        altar: 'Altar',
      },
      elementStoneVariants: {
        rubble: 'Rubble',
        rocks: 'Rocks',
        threeStones: 'Three stones',
        menhirs: 'Menhirs',
        largeStone: 'Large stone',
        mossyStone: 'Mossy stone',
        crackedStone: 'Cracked stone',
        magicStone: 'Magic stone',
        stoneWall: 'Stone wall',
      },
      elementMonsterVariants: {
        guardian: 'Guardian',
        knight: 'Knight',
        wizard: 'Wizard',
        monk: 'Monk',
        bard: 'Bard',
        priest: 'Priest',
        warlock: 'Warlock',
        hunter: 'Hunter',
        rogue: 'Rogue',
        paladin: 'Paladin',
        human: 'Human',
        elf: 'Elf',
        dwarf: 'Dwarf',
        halfling: 'Halfling',
        tiefling: 'Tiefling',
        draconian: 'Draconian',
        zombie: 'Zombie',
        goblin: 'Goblin',
        kobold: 'Kobold',
        thug: 'Thug',
        dragon: 'Dragon',
        wolf: 'Wolf',
        beast: 'Beast',
        demon: 'Demon',
        skeleton: 'Skeleton',
        ghost: 'Ghost',
        insect: 'Insect',
        mummy: 'Mummy',
        orc: 'Orc',
        golem: 'Golem',
      },
      elementMiscVariants: {
        chest: 'Chest',
        vase: 'Vase',
        woodenCrate: 'Wooden crate',
        campfire: 'Campfire',
        drainGrate: 'Drain grate',
        column: 'Column',
        brokenColumn: 'Damaged column',
        well: 'Well',
        fountain: 'Fountain',
        barrel: 'Barrel',
        signpost: 'Signpost',
        stalagmite: 'Stalagmite',
        stalactite: 'Stalactite',
        crystal: 'Crystal',
        statue: 'Statue',
        cart: 'Cart',
        camp: 'Camp',
        remainsCorpse: 'Remains/Corpse',
        haystack: 'Haystack',
      },
      fields: {
        name: 'Name',
        description: 'Description',
      },
      placeholders: {
        titleName: 'Unnamed map',
        description: 'Map description',
      },
    },
    contextList: {
      actions: {
        addContext: 'Add context',
      },
      emptyState: 'There are no contexts yet. Add one to start editing.',
      emptySearchState: 'No contexts match your search.',
      loading: 'Loading contexts...',
      searchLabel: 'Search contexts',
      searchPlaceholder: 'Search by context name',
      unnamedContext: 'Unnamed context',
      updatedAt: 'Last updated:',
      deleteDialog: {
        title: 'Delete this context?',
        body: 'The context "{name}" will be deleted permanently.',
      },
    },
    contextEdit: {
      eyebrow: 'CONTEXT',
      loading: 'Loading context...',
      copyButton: 'Copy context',
      copyingButton: 'Copying...',
      copySuccess: 'The context has been generated and copied to the clipboard.',
      copyError: 'The context could not be copied to the clipboard.',
      copy: {
        intro: 'I am presenting the situational context. Read it and wait for further instructions.',
        singleCharacterIntro: 'I am presenting the description of hero {name}. Read it and wait for further instructions.',
        singleNpcIntro: 'I am presenting the description of NPC {name}. Read it and wait for further instructions.',
        singleMonsterIntro: 'I am presenting the description of monster {name}. Read it and wait for further instructions.',
        singleAreaIntro: 'I am presenting the description of area {name}. Read it and wait for further instructions.',
        singleEventIntro: 'I am presenting the description of event {name}. Read it and wait for further instructions.',
        singleMapIntro: 'I am presenting the description of map {name}. Read it and wait for further instructions.',
        singleMapTitle: 'Map',
        heroesTitle: 'Hero party',
        npcGroupsTitle: 'NPC groups',
        monsterGroupsTitle: 'Monster groups',
        areasTitle: 'Areas where the action takes place',
        eventsTitle: 'Events related to the action',
        generalContextTitle: 'General context',
        levelLabel: 'level',
        alive: 'alive',
        dead: 'dead',
        characterHistoryTitle: 'Character history: {name}',
      },
      fields: {
        name: 'Title',
        description: 'Description',
        image: 'Context image',
      },
      placeholders: {
        titleName: 'Unnamed context',
        description: 'Context description',
        image: 'Add image',
      },
      imageActions: {
        uploadNew: 'Change',
        remove: 'Remove',
        removeDialog: {
          title: 'Remove context image?',
          body: 'The "{name}" will be removed from this context.',
        },
      },
      characters: {
        title: 'Heroes',
        emptyState: 'No hero groups added to this context yet.',
        addButton: 'Add hero groups',
        groupEmpty: 'No heroes in this group.',
        legacyGroupName: 'Ungrouped heroes',
        removeButton: 'Remove from context',
        removeGroupButton: 'Remove group from context',
        unknownCharacter: 'Unknown hero',
        addDialog: {
          title: 'Add hero groups to the context',
          searchLabel: 'Search hero group',
          searchPlaceholder: 'Type group name',
          emptyState: 'No hero groups available to add.',
          confirm: 'Add selected',
        },
      },
      events: {
        title: 'Events',
        emptyState: 'No events added to this context yet.',
        addButton: 'Add events',
        removeButton: 'Remove event from context',
        addDialog: {
          title: 'Add events to the context',
          searchLabel: 'Search event',
          searchPlaceholder: 'Type event name',
          emptyState: 'No events available to add.',
          confirm: 'Add selected',
        },
      },
      npcGroups: {
        title: 'NPC groups',
        emptyState: 'No NPC groups added to this context yet.',
        addButton: 'Add NPC groups',
        removeGroupButton: 'Remove group from context',
        removeNpcButton: 'Remove NPC from group',
        groupEmpty: 'No NPCs in this group.',
        addDialog: {
          title: 'Add NPC groups to the context',
          searchLabel: 'Search group',
          searchPlaceholder: 'Type group name',
          emptyState: 'No NPC groups available to add.',
          confirm: 'Add selected',
        },
      },
      monsterGroups: {
        title: 'Monster groups',
        emptyState: 'No monster groups added to this context yet.',
        addButton: 'Add monster groups',
        removeGroupButton: 'Remove group from context',
        removeMonsterButton: 'Remove monster from group',
        groupEmpty: 'No monsters in this group.',
        addDialog: {
          title: 'Add monster groups to the context',
          searchLabel: 'Search group',
          searchPlaceholder: 'Type group name',
          emptyState: 'No monster groups available to add.',
          confirm: 'Add selected',
        },
      },
      areas: {
        title: 'Areas',
        emptyState: 'No areas added to this context yet.',
        addButton: 'Add areas',
        removeAreaButton: 'Remove area from context',
        removePlaceButton: 'Remove place from area',
        areaEmpty: 'No places in this area.',
        placeCount: 'Places: {count}',
        addDialog: {
          title: 'Add areas to the context',
          searchLabel: 'Search area',
          searchPlaceholder: 'Type area name',
          emptyState: 'No areas available to add.',
          confirm: 'Add selected',
        },
      },
    },
    areaEdit: {
      contextCopyError: 'Could not copy area context.',
      contextCopySuccess: 'Area context copied.',
      copyingContextButton: 'Copying...',
      copyContextButton: 'Context',
      eyebrow: 'AREA',
      loading: 'Loading area...',
      fields: {
        name: 'Name',
        description: 'Description',
        image: 'Area image',
      },
      placeholders: {
        titleName: 'Unnamed area',
        description: 'Area description',
        image: 'Add image',
      },
      imageActions: {
        uploadNew: 'Change',
        remove: 'Remove',
        removeDialog: {
          title: 'Remove area image?',
          body: 'The "{name}" will be removed from this area.',
        },
      },
      places: {
        title: 'Places',
        emptyState: 'No places have been added yet.',
        addButton: 'Add place',
        removeButton: 'Remove',
        nameLabel: 'Place name',
        namePlaceholder: 'e.g. The Golden Dragon Inn',
        descriptionLabel: 'Place description',
        descriptionPlaceholder: 'Short place description',
        unnamedItem: 'Unnamed place',
        removeDialog: {
          title: 'Delete this place?',
          body: 'The place "{name}" will be deleted permanently.',
        },
      },
    },
    adventureList: {
      actions: {
        addAdventure: 'Add adventure',
      },
      emptyState: 'There are no adventures yet. Add one to start editing.',
      loading: 'Loading adventures...',
      unnamedAdventure: 'Unnamed adventure',
      updatedAt: 'Last updated:',
    },
    adventureEdit: {
      eyebrow: 'ADVENTURE',
      loading: 'Loading adventure...',
      actions: {
        send: 'Send',
        sending: 'Sending...',
      },
      fields: {
        name: 'Name',
        output: 'Output',
        prompt: 'Prompt',
      },
      placeholders: {
        output: 'The game master response will appear here',
        titleName: 'Unnamed adventure',
        prompt: 'Enter the prompt for preparing the adventure',
      },
      tokens: {
        lastUsage: 'Last: {input} input / {output} output / {total} total',
        noLastUsage: 'Last: no request sent',
        promptCount: 'Prompt: {count} tokens',
        promptUnavailable: 'Prompt: tokens unavailable',
        quotaReset: 'Daily quota resets at midnight Pacific Time',
        summaryLabel: 'Gemini usage',
        todayRequests: 'Requests today: {count}',
      },
    },
    characterList: {
      actions: {
        addHero: 'Add hero',
        addMonster: 'Add monster',
        addGroup: 'Add group',
      },
      emptyState: 'There are no character sheets yet. Add one to start editing.',
      emptySearchState: 'No heroes match your search.',
      searchLabel: 'Search heroes',
      searchPlaceholder: 'Search by hero name',
      unnamedCharacter: 'Nameless hero',
      missingRace: 'No race',
      missingClass: 'No class',
      missingUpdatedAt: 'Never saved',
      tabs: {
        groups: 'Groups',
        list: 'List',
      },
      groups: {
        loading: 'Loading hero groups...',
        emptyState: 'There are no hero groups yet.',
        emptySearchState: 'No groups match your search.',
        searchLabel: 'Search hero groups',
        searchPlaceholder: 'Search by group or hero name',
        emptyCharacters: 'No heroes',
        characterCount: 'Heroes: {count}',
        moreCharacters: 'More heroes',
        unnamedGroup: 'Unnamed group',
        deleteDialog: {
          title: 'Delete this hero group?',
          body: 'The group "{name}" will be deleted permanently.',
        },
        createDialog: {
          title: 'Add group',
          nameLabel: 'Group name',
          namePlaceholder: 'e.g. Fallcrest party',
        },
      },
      deleteDialog: {
        title: 'Delete this sheet?',
        body: 'The sheet "{name}" will be deleted permanently.',
      },
    },
    characterGroupEdit: {
      eyebrow: 'HERO GROUP',
      loading: 'Loading hero group...',
      emptyState: 'No heroes have been added to this group yet.',
      actions: {
        addCharacter: 'Add hero',
        createCharacterInGroup: 'Create hero in this group',
        removeCharacter: 'Remove from group',
      },
      fields: {
        name: 'Group name',
      },
      placeholders: {
        name: 'Group name',
      },
      sections: {
        availableCharacters: 'Available heroes',
        characters: 'Heroes in group',
      },
      addCharacterDialog: {
        searchLabel: 'Search hero',
        searchPlaceholder: 'Type at least 3 characters',
        emptyState: 'There are no heroes to add.',
      },
    },
    monsterList: {
      actions: {
        addGroup: 'Add group',
      },
      tabs: {
        groups: 'Groups',
        list: 'List',
      },
      groups: {
        addMonster: 'Add monster to group',
        loading: 'Loading monster groups...',
        emptyState: 'There are no monster groups yet.',
        emptySearchState: 'No groups match your search.',
        searchLabel: 'Search monster groups',
        searchPlaceholder: 'Search by group or monster name',
        emptyMonsters: 'No monsters',
        monsterCount: 'Monsters: {count}',
        moreMonsters: 'More monsters',
        unnamedGroup: 'Unnamed group',
        uniqueId: 'ID:',
        deleteDialog: {
          title: 'Delete this monster group?',
          body: 'The group "{name}" will be deleted permanently.',
        },
        createDialog: {
          title: 'Add group',
          nameLabel: 'Group name',
          namePlaceholder: 'e.g. Ruin guards',
        },
        addMonsterDialog: {
          title: 'Add monster to "{name}"',
          searchLabel: 'Search monster',
          searchPlaceholder: 'Type at least 3 characters',
          emptyState: 'There are no monsters to add.',
        },
      },
      emptyState: 'There are no monsters yet.',
      emptySearchState: 'No monsters match your search.',
      searchLabel: 'Search monsters',
      searchPlaceholder: 'Search by monster name',
      unnamedMonster: 'Nameless monster',
      deleteDialog: {
        title: 'Delete this monster?',
        body: 'The monster "{name}" will be deleted permanently.',
      },
    },
    npcList: {
      actions: {
        addNpc: 'Add NPC',
        addGroup: 'Add group',
      },
      tabs: {
        groups: 'Groups',
        list: 'List',
      },
      groups: {
        addNpc: 'Add NPC to group',
        loading: 'Loading NPC groups...',
        emptyState: 'There are no NPC groups yet.',
        emptySearchState: 'No groups match your search.',
        searchLabel: 'Search NPC groups',
        searchPlaceholder: 'Search by group or NPC name',
        emptyNpcs: 'No NPC',
        npcCount: 'NPC: {count}',
        moreNpcs: 'More NPC',
        unnamedGroup: 'Unnamed group',
        uniqueId: 'ID:',
        deleteDialog: {
          title: 'Delete this NPC group?',
          body: 'The group "{name}" will be deleted permanently.',
        },
        createDialog: {
          title: 'Add group',
          nameLabel: 'Group name',
          namePlaceholder: 'e.g. Ruin guards',
        },
        addNpcDialog: {
          title: 'Add NPC to "{name}"',
          searchLabel: 'Search NPC',
          searchPlaceholder: 'Type at least 3 characters',
          emptyState: 'There are no NPC to add.',
        },
      },
      emptyState: 'There are no NPC yet.',
      emptySearchState: 'No NPC match your search.',
      searchLabel: 'Search NPC',
      searchPlaceholder: 'Search by NPC name',
      unnamedNpc: 'Nameless NPC',
      deleteDialog: {
        title: 'Delete this NPC?',
        body: 'The NPC "{name}" will be deleted permanently.',
      },
    },
    monsterEdit: {
      contextCopyError: 'Could not copy monster context.',
      contextCopySuccess: 'Monster context copied.',
      copyingContextButton: 'Copying...',
      copyContextButton: 'Context',
      eyebrow: 'MONSTER',
      title: 'Monster',
      loading: 'Loading monster data...',
      tabs: {
        general: 'Main',
        attacks: 'Attacks',
        loot: 'Loot',
      },
      sections: {
        defenses: 'Defenses',
        suggested: 'Suggested',
        description: 'Description',
        resistances: 'Resistances',
        special: 'Special',
      },
      imageActions: {
        uploadNew: 'New',
        remove: 'Remove',
      },
      actions: {
        generateAttributes: 'Generate attributes',
      },
      generateAttributesDialog: {
        title: 'Generate attributes?',
        body: 'This will replace HP, defenses, and suggested attack and damage values for the current monster level, role, and type.',
        confirm: 'Generate',
      },
      attacks: {
        title: 'Attacks',
        emptyState: 'No attacks have been added yet.',
        addButton: 'Add attack',
        removeButton: 'Remove',
        typeLabel: 'Attack type',
        typeOptions: {
          standard: 'Standard',
          unlimited: 'Unlimited',
          encounter: 'Encounter',
          daily: 'Daily',
        },
        attackLabel: 'Attack',
        attackSuggestion: 'Attack: vs AC: {attackVsKp}, vs other defenses {attackVsOtherDefenses}',
        damageSuggestion: 'Low damage: {lowDamage}, medium: {mediumDamage}, high {highDamage}',
        namePlaceholder: 'e.g. Claws',
        descriptionPlaceholder: 'Attack effect description',
        notApplicable: 'not applicable',
      },
      loot: {
        title: 'Loot',
        emptyState: 'No loot has been added yet.',
        addButton: 'Add loot',
        groups: {
          armors: 'Armors',
          weapons: 'Weapons',
          others: 'Others',
        },
        removeDialog: {
          title: 'Delete this loot?',
          body: 'The loot "{name}" will be deleted permanently.',
        },
      },
      typeOptions: {
        minion: 'Minion',
        normal: 'Normal',
        solo: 'Solo',
        elite: 'Elite',
      },
      roleOptions: {
        skirmisher: 'Skirmisher',
        brute: 'Brute',
        soldier: 'Soldier',
        lurker: 'Lurker',
        controller: 'Controller',
        artillery: 'Artillery',
      },
      fields: {
        name: 'Name',
        image: 'Monster image',
        description: 'Description',
        kp: 'AC',
        fortitude: 'Fortitude',
        reflex: 'Reflex',
        will: 'Will',
        speed: 'Speed',
        role: 'Role',
        type: 'Type',
        hp: 'HP',
        level: 'Level',
        bloodied: 'Bloodied',
        attack: 'Attack:',
        damage: 'Dmg:',
        vsKp: 'vs AC',
        vsOtherDefenses: 'vs other defenses',
        low: '[S]',
        medium: '[M]',
        high: '[L]',
        custom: '[C]',
        attackVsKp: 'Attack vs AC',
        attackVsOtherDefenses: 'Attack vs other defenses',
        lowDamage: 'Low damage',
        mediumDamage: 'Medium damage',
        highDamage: 'High damage',
        customDamage: 'Custom damage',
      },
      placeholders: {
        titleName: 'Nameless monster',
        image: 'Add image',
        description: 'Monster description',
        resistances: 'Resistances, immunities, and vulnerabilities',
        special: 'Special monster traits',
      },
    },
    npcEdit: {
      contextCopyError: 'Could not copy NPC context.',
      contextCopySuccess: 'NPC context copied.',
      copyingContextButton: 'Copying...',
      copyContextButton: 'Context',
      eyebrow: 'NPC',
      title: 'NPC',
      loading: 'Loading NPC data...',
      tabs: {
        general: 'Main',
        attacks: 'Attacks',
        loot: 'Loot',
        history: 'History',
      },
      sections: {
        defenses: 'Defenses',
        suggested: 'Suggested',
        description: 'Description',
        resistances: 'Resistances',
        special: 'Special',
      },
      imageActions: {
        uploadNew: 'New',
        remove: 'Remove',
      },
      actions: {
        generateAttributes: 'Generate attributes',
      },
      generateAttributesDialog: {
        title: 'Generate attributes?',
        body: 'This will replace HP, defenses, and suggested attack and damage values for the current NPC level, role, and type.',
        confirm: 'Generate',
      },
      attacks: {
        title: 'Attacks',
        emptyState: 'No attacks have been added yet.',
        addButton: 'Add attack',
        removeButton: 'Remove',
        typeLabel: 'Attack type',
        typeOptions: {
          standard: 'Standard',
          unlimited: 'Unlimited',
          encounter: 'Encounter',
          daily: 'Daily',
        },
        attackLabel: 'Attack',
        attackSuggestion: 'Attack: vs AC: {attackVsKp}, vs other defenses {attackVsOtherDefenses}',
        damageSuggestion: 'Low damage: {lowDamage}, medium: {mediumDamage}, high {highDamage}',
        namePlaceholder: 'e.g. Claws',
        descriptionPlaceholder: 'Attack effect description',
        notApplicable: 'not applicable',
      },
      loot: {
        title: 'Loot',
        emptyState: 'No loot has been added yet.',
        addButton: 'Add loot',
        groups: {
          armors: 'Armors',
          weapons: 'Weapons',
          others: 'Others',
        },
        removeDialog: {
          title: 'Delete this loot?',
          body: 'The loot "{name}" will be deleted permanently.',
        },
      },
      history: {
        title: 'History',
        emptyState: 'No history entries have been added yet.',
        addButton: 'Add history entry',
        removeButton: 'Remove',
        titlePlaceholder: 'Entry title',
        contentLabel: 'Content',
        contentPlaceholder: 'Describe a part of the NPC history',
        removeDialog: {
          title: 'Delete this history entry?',
          body: 'The entry "{name}" will be deleted permanently.',
        },
      },
      typeOptions: {
        minion: 'Minion',
        normal: 'Normal',
        solo: 'Solo',
        elite: 'Elite',
      },
      roleOptions: {
        skirmisher: 'Skirmisher',
        brute: 'Brute',
        soldier: 'Soldier',
        lurker: 'Lurker',
        controller: 'Controller',
        artillery: 'Artillery',
      },
      fields: {
        name: 'Name',
        image: 'NPC image',
        description: 'Description',
        kp: 'AC',
        fortitude: 'Fortitude',
        reflex: 'Reflex',
        will: 'Will',
        speed: 'Speed',
        role: 'Role',
        type: 'Type',
        hp: 'HP',
        level: 'Level',
        bloodied: 'Bloodied',
        attack: 'Attack:',
        damage: 'Dmg:',
        vsKp: 'vs AC',
        vsOtherDefenses: 'vs other defenses',
        low: '[S]',
        medium: '[M]',
        high: '[L]',
        custom: '[C]',
        attackVsKp: 'Attack vs AC',
        attackVsOtherDefenses: 'Attack vs other defenses',
        lowDamage: 'Low damage',
        mediumDamage: 'Medium damage',
        highDamage: 'High damage',
        customDamage: 'Custom damage',
        isStory: 'Story NPC',
        isDead: 'Dead',
      },
      placeholders: {
        titleName: 'Nameless NPC',
        image: 'Add image',
        description: 'NPC description',
        resistances: 'Resistances, immunities, and vulnerabilities',
        special: 'Special NPC traits',
      },
    },
    monsterGroupEdit: {
      eyebrow: 'MONSTER GROUP',
      loading: 'Loading monster group...',
      emptyState: 'No monsters have been added to this group yet.',
      actions: {
        addMonster: 'Add monster',
        createMonsterInGroup: 'Create monster in this group',
        removeMonster: 'Remove from group',
      },
      fields: {
        name: 'Group name',
      },
      placeholders: {
        name: 'Group name',
      },
      sections: {
        availableMonsters: 'Available monsters',
        monsters: 'Monsters in group',
      },
      addMonsterDialog: {
        title: 'Add monster to group',
        searchLabel: 'Search monster',
        searchPlaceholder: 'Type at least 3 characters',
        emptyState: 'There are no monsters to add.',
      },
    },
    npcGroupEdit: {
      eyebrow: 'NPC GROUP',
      loading: 'Loading NPC group...',
      emptyState: 'No NPC have been added to this group yet.',
      actions: {
        addNpc: 'Add NPC',
        createNpcInGroup: 'Create NPC in this group',
        removeNpc: 'Remove from group',
      },
      fields: {
        name: 'Group name',
      },
      placeholders: {
        name: 'Group name',
      },
      sections: {
        availableNpcs: 'Available NPC',
        npcs: 'NPC in group',
      },
      addNpcDialog: {
        title: 'Add NPC to group',
        searchLabel: 'Search NPC',
        searchPlaceholder: 'Type at least 3 characters',
        emptyState: 'There are no NPC to add.',
      },
    },
    characterEdit: {
      contextCopyError: 'Could not copy hero context.',
      contextCopySuccess: 'Hero context copied.',
      copyingContextButton: 'Copying...',
      copyContextButton: 'Context',
      eyebrow: 'CHARACTER SHEET',
      title: 'Hero',
      tabs: {
        general: 'Main',
        abilities: 'Abilities',
        feats: 'Feats',
        items: 'Items',
        history: 'History',
      },
      printMenu: {
        characterSheet: 'Character sheet',
        abilitiesAndFeats: 'Abilities and feats',
        items: 'Items',
      },
      imageActions: {
        uploadNew: 'New',
        remove: 'Remove',
      },
      sections: {
        general: 'General',
        attributes: 'Attributes',
        defenses: 'Defenses',
        description: 'Description',
        skills: 'Skills (+ training)',
      },
      defenseTooltip: {
        levelBonus: 'Level bonus',
        classBonus: 'Class bonus',
        attributesBonus: 'Attribute bonus',
        itemsBonus: 'Item bonuses:',
      },
      skillTooltip: {
        trainingBonus: 'training bonus',
        armorPenalty: 'Armor penalty',
      },
      sourceTooltip: {
        raceBonus: 'Race bonus',
        itemBonus: 'Item bonuses:',
        featBonuses: 'Feats:',
        baseSpeed: 'Base speed from race',
      },
      abilities: {
        title: 'Abilities',
        emptyState: 'No abilities have been added yet.',
        addTitle: 'Add ability',
        addButton: 'Add ability',
        removeButton: 'Remove',
        removeConfirm: 'Remove this ability?',
        removeDialog: {
          title: 'Delete this ability?',
          body: 'The ability "{name}" will be deleted permanently.',
        },
        actionLabel: 'Action type',
        areaLabel: 'Area',
        actionOptions: {
          action: 'Action',
          noAction: 'No action',
        },
        weaponRangeLabel: 'Distance',
        weaponAreaLabel: 'Area',
        typeLabel: 'Type',
        typeOptions: {
          standard: 'Standard',
          unlimited: 'Unlimited',
          encounter: 'Encounter',
          daily: 'Daily',
        },
        kindLabel: 'Kind',
        kindOptions: {
          offensive: 'Offensive',
          utility: 'Utility',
        },
        weaponAttackAttributeLabel: 'Attack attribute',
        weaponAttackBonusLabel: 'Bonus',
        weaponAgainstLabel: 'against',
        weaponAttackDefenseLabel: 'Defense',
        weaponLabel: 'Single-use damage',
        attackLabel: 'Attack',
        weaponDamageDiceLabel: 'Die',
        weaponDamageCountLabel: 'Count',
        weaponDamageTypeLabel: 'Damage type',
        weaponRecurringDamageLabel: 'Recurring damage',
        weaponBonusOptions: {
          none: 'None',
          prefix: 'bonus',
        },
        weaponDamageDiceOptions: {
          none: 'None',
          d4: 'K4',
          d6: 'K6',
          d8: 'K8',
          d10: 'K10',
          d12: 'K12',
          d20: 'K20',
        },
        weaponDamageTypeOptions: {
          normal: 'Normal',
          acid: 'Acid',
          cold: 'Cold',
          fire: 'Fire',
          force: 'Force',
          lightning: 'Lightning',
          necrotic: 'Necrotic',
          poison: 'Poison',
          psychic: 'Psychic',
          radiant: 'Radiant',
          thunder: 'Thunder',
        },
        weaponAreaOptions: {
          point: 'Point',
          burst: 'Burst',
          blast: 'Blast',
        },
        weaponHitLabel: 'Hit',
        weaponMissLabel: 'Miss',
        weaponProvocationLabel: 'Provocation',
        weaponOptions: {
          none: 'None',
        },
        nameLabel: 'Name',
        descriptionLabel: 'Description',
        namePlaceholder: 'e.g. Shield Bash',
        descriptionPlaceholder: 'Short ability description',
      },
      feats: {
        title: 'Feats',
        emptyState: 'No feats have been added yet.',
        addButton: 'Add feat',
        removeButton: 'Remove',
        visibleLabel: 'Visible',
        invisibleLabel: 'Invisible',
        removeDialog: {
          title: 'Delete this feat?',
          body: 'The feat "{name}" will be deleted permanently.',
        },
        nameLabel: 'Name',
        namePlaceholder: 'e.g. Cold Blood',
        descriptionLabel: 'Description',
        descriptionPlaceholder: 'Short feat description',
      },
      items: {
        title: 'Items',
        emptyState: 'No items have been added yet.',
        addButton: 'Add item',
        removeButton: 'Remove',
        groups: {
          armors: 'Armors',
          weapons: 'Weapons',
          others: 'Others',
        },
        weaponBonus: {
          none: 'None',
        },
        removeDialog: {
          title: 'Delete this item?',
          body: 'The item "{name}" will be deleted permanently.',
        },
        nameLabel: 'Name',
        descriptionLabel: 'Description',
        damageLabel: 'Damage',
        rangeLabel: 'Range',
        proficiencyLabel: 'Proficiency +',
        bonusesLabel: 'Bonuses',
        addBonusButton: 'Add bonus',
        bonusTypeLabel: 'Bonus type',
        bonusValueLabel: 'Bonus value',
        equippedLabel: 'Equipped',
        unequippedLabel: 'Unequipped',
        namePlaceholder: 'e.g. Healing potion',
        descriptionPlaceholder: 'Short item description',
      },
      history: {
        title: 'History',
        emptyState: 'No history entries have been added yet.',
        addButton: 'Add history entry',
        removeButton: 'Remove',
        titlePlaceholder: 'e.g. Fall of the house',
        contentLabel: 'Content',
        contentPlaceholder: 'Describe a part of the hero history',
        removeDialog: {
          title: 'Delete this history entry?',
          body: 'The entry "{name}" will be deleted permanently.',
        },
      },
      fields: {
        name: 'Name',
        image: 'Hero image',
        description: 'Description',
        shortDescription: 'Short description',
        longDescription: 'Long description',
        level: 'Level',
        speed: 'Speed',
        hp: 'HP',
        surge: 'Healing Surge',
        race: 'Race',
        class: 'Class',
        gender: 'Gender',
        alignment: 'Alignment',
        strength: 'Strength',
        condition: 'Condition',
        dexterity: 'Dexterity',
        intelligence: 'Intelligence',
        wisdom: 'Wisdom',
        charisma: 'Charisma',
        kp: 'AC',
        fortitude: 'Fortitude',
        reflex: 'Reflex',
        will: 'Will',
        armorPenalty: 'Armor penalty',
        acrobatics: 'Acrobatics',
        arcana: 'Arcana',
        athletics: 'Athletics',
        diplomacy: 'Diplomacy',
        history: 'History',
        healing: 'Heal',
        deception: 'Bluff',
        perception: 'Perception',
        endurance: 'Endurance',
        dungeons: 'Dungeons',
        nature: 'Nature',
        religion: 'Religion',
        insight: 'Insight',
        stealth: 'Stealth',
        streetwise: 'Streetwise',
        intimidation: 'Intimidate',
        thievery: 'Thievery',
      },
      placeholders: {
        name: 'e.g. Seraphina',
        description: 'Character description',
        shortDescription: 'Short character description',
        longDescription: 'Long character description',
        titleName: 'Nameless hero',
        race: 'Select race',
        class: 'Select class',
      },
      options: {
        race: {
          human: 'Human',
          tiefling: 'Tiefling',
          dragonborn: 'Dragonborn',
          eladrin: 'Eladrin',
          elf: 'Elf',
          dwarf: 'Dwarf',
          halfling: 'Halfling',
          halfElf: 'Half-elf',
        },
        class: {
          warlock: 'Warlock',
          wizard: 'Wizard',
          warlord: 'Warlord',
          bard: 'Bard',
          cleric: 'Cleric',
          rogue: 'Rogue',
          ranger: 'Ranger',
          paladin: 'Paladin',
          fighter: 'Fighter',
          barbarian: 'Barbarian',
        },
        gender: {
          male: 'Male',
          female: 'Female',
          unspecified: 'Unspecified',
        },
        alignment: {
          lawfulGood: 'Lawful Good',
          lawfulNeutral: 'Lawful Neutral',
          lawfulEvil: 'Lawful Evil',
          neutralGood: 'Neutral Good',
          trueNeutral: 'Neutral',
          neutralEvil: 'Neutral Evil',
          chaoticGood: 'Chaotic Good',
          chaoticNeutral: 'Chaotic Neutral',
          chaoticEvil: 'Chaotic Evil',
        },
      },
    },
    characterPrint: {
      title: 'Character sheet',
      loading: 'Preparing print view...',
      error: 'Could not prepare the print view.',
      pdLabel: 'PD',
      printButtonLabel: 'Print',
    },
    characterAbilitiesPrint: {
      title: 'Abilities and feats',
      loading: 'Preparing abilities print view...',
      error: 'Could not prepare the abilities print view.',
      printButtonLabel: 'Print',
      emptyState: 'No abilities have been added yet.',
      attackLabel: 'Attack',
      damageLabel: 'Damage',
      damageParts: {
        recurring: 'recurring',
        type: 'type',
      },
      sections: {
        standard: 'Standard',
        unlimited: 'Unlimited',
        encounter: 'Encounter',
        daily: 'Daily',
        feats: 'Feats',
      },
    },
    characterItemsPrint: {
      title: 'Items',
      loading: 'Preparing items print view...',
      error: 'Could not prepare the items print view.',
      printButtonLabel: 'Print',
      emptyState: 'No items have been added yet.',
      extraSlots: {
        goldCoins: 'Gold coins',
      },
      sections: {
        armors: 'Armors',
        weapons: 'Weapons',
        others: 'Others',
      },
    },
    mapPrint: {
      title: 'Map print',
      loading: 'Preparing map print view...',
      error: 'Could not prepare the map print view.',
      printButtonLabel: 'Print',
      mapLabel: 'Map',
      emptyLegend: 'No map labels have been added yet.',
      sections: {
        description: 'Description',
        legend: 'Legend',
      },
    },
    monsterPrint: {
      title: 'Monster sheet',
      loading: 'Preparing monster print view...',
      error: 'Could not prepare the monster print view.',
      printButtonLabel: 'Print',
      emptyAttacks: 'No attacks have been added yet.',
      unnamedAttack: 'Unnamed attack',
      sections: {
        stats: 'Level, speed, and HP',
        attacks: 'Attacks',
        items: 'Items',
        weapons: 'Weapons',
        armors: 'Armors',
        others: 'Others',
      },
    },
    npcPrint: {
      title: 'NPC sheet',
      loading: 'Preparing NPC print view...',
      error: 'Could not prepare the NPC print view.',
      printButtonLabel: 'Print',
      emptyAttacks: 'No attacks have been added yet.',
      unnamedAttack: 'Unnamed attack',
      sections: {
        stats: 'Level, speed, and HP',
        attacks: 'Attacks',
        items: 'Items',
        weapons: 'Weapons',
        armors: 'Armors',
        others: 'Others',
      },
    },
  },
}
