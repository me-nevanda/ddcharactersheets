...
const normalizeSpeedValue = (value: unknown): number => {
    if (typeof value !== 'number' || !Number.isFinite(value)) {
        return 6;
    }
    return Math.min(12, Math.max(1, Math.trunc(value)));
};
const normalizeReadOnlyValue = (value: unknown): number => {
    if (typeof value !== 'number' || !Number.isFinite(value)) {
        return 0;
    }
...
const normalizeDefenses = (data: Partial<Record<keyof CharacterDefenses, unknown>> = {}): CharacterDefenses => {
    return {
...
const normalizeTrainingValue = (value: unknown): boolean => {
    return value === true;
};
const normalizeAbilityWeaponDamageDiceType = (value: unknown): CharacterWeaponDamageDiceType | '' => {
    if (value === '' ||
        value === 'd4' ||
        value === 'd6' ||
        value === 'd8' ||
...
const normalizeAbilityType = (value: unknown): CharacterAbilityType => {
    if (value === 'standard' ||
        value === 'unlimited' ||
        value === 'encounter' ||
        value === 'daily') {
...
const normalizeAbilities = (data: unknown, weapons: CharacterWeapon[] = []): CharacterAbility[] => {
    ...
    return data
        ...
        weaponRecurringDamageCount: typeof item.weaponRecurringDamageCount === 'number' && Number.isFinite(item.weaponRecurringDamageCount)
...
const normalizeWeaponAttributeBonus = (value: unknown): keyof CharacterAttributes | '' => {
    if (value === 'strength' ||
        value === 'condition' ||
        value === 'dexterity' ||
        value === 'intelligence' ||
...
const normalizeItemGroup = <T extends CharacterArmor | CharacterWeapon | CharacterOtherItem>(data: unknown): T[] => {
    if (!Array.isArray(data)) {
        return [];
    }
...
const normalizeArmorGroup = (data: unknown): CharacterArmor[] => {
    if (!Array.isArray(data)) {
        return [];
    }
    return data
...
const normalizeWeaponGroup = (data: unknown): CharacterWeapon[] => {
    if (!Array.isArray(data)) {
        return [];
    }
    return data
...
const normalizeItems = (data: unknown): {
...
const getLevelBonus = (value: number): number => {
    return Math.floor(normalizeLevelValue(value) / 2);
};
const buildAttributeBonuses = (attributes: CharacterAttributes): CharacterAttributeBonuses => {
    return {
...
const buildZeroAttributeBonuses = (): CharacterAttributeBonuses => {
    return {
        strength: 0,
        condition: 0,
        dexterity: 0,
...
const normalizeAttributeBonusValue = (value: unknown): number | null => {
    if (typeof value !== 'number' || !Number.isFinite(value)) {
        return null;
    }
    return Math.trunc(value);
};
const normalizeAttributeBonuses = (data: Partial<Record<keyof CharacterAttributeBonuses, unknown>> | undefined, fallback: CharacterAttributeBonuses): CharacterAttributeBonuses => {
    return {
...
const normalizeDefenseBonusValue = (value: unknown): number | null => {
    if (typeof value !== 'number' || !Number.isFinite(value)) {
        return null;
    }
    return Math.trunc(value);
};
const normalizeDefenseBonuses = (data: Partial<Record<keyof CharacterDefenseBonuses, unknown>> | undefined, fallback: CharacterDefenseBonuses): CharacterDefenseBonuses => {
    return {
...
const buildSkillBonuses = (level: number, attributeBonuses: CharacterAttributeBonuses, training: CharacterTraining): CharacterSkillBonuses => {
    const levelBonus = getLevelBonus(level);
    const withTraining = (baseBonus: number, trained: boolean): number => {
...
const normalizeSkillBonusValue = (value: unknown): number | null => {
    if (typeof value !== 'number' || !Number.isFinite(value)) {
        return null;
    }
    return Math.trunc(value);
};
const normalizeSkillBonuses = (data: Partial<Record<keyof CharacterSkillBonuses, unknown>> | undefined, fallback: CharacterSkillBonuses): CharacterSkillBonuses => {
    return {
...
const buildDefenseBonuses = (level: number, attributeBonuses: CharacterAttributeBonuses): CharacterDefenseBonuses => {
    const levelBonus = getLevelBonus(level);
    return {
...
const normalizeTraining = (data: Partial<Record<keyof CharacterTraining, unknown>> = {}): CharacterTraining => {
    return {
...
const normalizeCharacter = (data: Partial<Record<keyof CharacterData, unknown>> = {}): CharacterData => {
    ...
    const bonusData = typeof data.bonuses === 'object' && data.bonuses !== null
        ? (data.bonuses as LegacyCharacterBonuses)
        : undefined;
    const attributesPlusData = typeof data.attributesPlus === 'object' && data.attributesPlus !== null
    ...
    const bonuses: CharacterBonuses = {
        ...
        attributes: normalizeAttributeBonuses(bonusData?.attributes as Partial<Record<keyof CharacterAttributeBonuses, unknown>> | undefined, computedAttributeBonuses),
        skills: normalizeSkillBonuses(bonusData?.skills as Partial<Record<keyof CharacterSkillBonuses, unknown>> | undefined, computedSkillBonuses),
    ...
    return {
        ...
        items,
        bonuses,
        defenses: typeof data.defenses === 'object' && data.defenses !== null
...
export const listCharacters = async (): Promise<Character[]> => {
    ...
    const characters = await listStoredCharacters<CharacterData, Character>(characterStoreOptions);
...
export const readCharacter = async (characterId: string): Promise<Character> => {
    ...
    const [character, imageInfo] = await Promise.all([
        readStoredCharacter<CharacterData, Character>(characterId, characterStoreOptions),
...
export const createCharacter = async (): Promise<Character> => {
    await ensureCharactersStore();
    return createStoredCharacter<CharacterData, Character>(characterStoreOptions);
};
...
export const deleteCharacter = async (characterId: string): Promise<void> => {
    await ensureCharactersStore();
    await deleteStoredEntity(characterStoreOptions.tableName, characterId);
...
export const updateCharacterImage = async (characterId: string, contentType: string | undefined, data: Buffer): Promise<Character> => {
    await ensureCharactersStore();
    await assertStoredEntityExists(characterStoreOptions.tableName, characterId);
